...
function ComposerPerform.get_velocity_multiplier(lane_id)
  ...
  if mode == MODE_MUTE then
    target = 0.0
    slew_time = params:get("lane_" .. lane_id .. "_composer_performance_mute_slew")
  elseif mode == MODE_ACCENT then
    target = params:get("lane_" .. lane_id .. "_composer_performance_accent_amount")
    slew_time = params:get("lane_" .. lane_id .. "_composer_performance_slew")
...
local function create_params()
  for i = 1, 8 do
    ...
    params:set_action("lane_" .. i .. "_composer_performance_mode", function(value)
      if _seeker and _seeker.composer and _seeker.composer.perform and _seeker.composer.perform.screen then
        _seeker.composer.perform.screen:rebuild_params()
      end
    ...
    params:add_control("lane_" .. i .. "_composer_performance_accent_amount", "Accent Multiplier",
      controlspec.new(1.0, 2.0, 'lin', 0.1, 1.5, "x"))

    params:add_control("lane_" .. i .. "_composer_performance_soft_amount", "Soft Multiplier",
    ...
    params:add_control("lane_" .. i .. "_composer_performance_mute_slew", "Mute Slew",
      controlspec.new(0.0, 5.0, 'lin', 0.01, 0.0, "s"))
  end
end
...
local function create_screen_ui()
  ...
  norns_ui.rebuild_params = function(self)
    ...
    if mode == MODE_MUTE then
      table.insert(param_table, { id = "lane_" .. lane_id .. "_composer_performance_mute_slew", arc_multi_float = {1.0, 0.1, 0.01} })
    elseif mode == MODE_ACCENT then
      table.insert(param_table, { id = "lane_" .. lane_id .. "_composer_performance_accent_amount", arc_multi_float = {0.1, 0.05, 0.01} })
      table.insert(param_table, { id = "lane_" .. lane_id .. "_composer_performance_slew", arc_multi_float = {1.0, 0.1, 0.01} })
    elseif mode == MODE_SOFT then
