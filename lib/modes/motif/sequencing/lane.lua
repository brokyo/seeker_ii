...
---------------------------------------------------------
function Lane:schedule_stage(stage_index, start_time, resume_from)
  ...
  for i, event in ipairs(self.motif.events) do
    ...
    if event.type == "note_on" then
      ...
      if event_time <= base_duration then
        ...
        if resume_from and absolute_time < resume_from then
          goto continue
        end
...
function Lane:on_note_on(event)
  ...
  if self.txo_osc_active then
    ...
    if osc_num then  -- nil if all outputs claimed by CV
