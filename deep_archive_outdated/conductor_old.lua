...
function Lane.new(lane_num)
  ...
  l.active_notes = {}
  l.initial_sync_done = false  -- Track whether we've done initial sync
  l.playback_start_beat = nil  -- Track absolute start beat
  l.next_stage = nil          -- Queue for next stage
...
function Lane:advance_stage()
  ...
  if transform_config.type ~= "none" then
    ...
    if transform and transform.fn then
      Log.log("CONDUCTOR", "TRANSFORM", string.format("%s Applying transform %s | Lane %d Stage %d", Log.ICONS.TRANSFORM, transform_config.type, self.lane_num, self.current_stage))
...
function Conductor:play_note_on(lane, note)
  ...
  Log.log("CONDUCTOR", "PLAYBACK", string.format("%s Note ON | Pitch %d | Beat %s | Lane %d", Log.ICONS.NOTE_ON, note.pitch, Log.format.beat(note.time), lane.lane_num))
...
function Conductor:play_note_off(lane, note)
  ...
  Log.log("CONDUCTOR", "PLAYBACK", string.format("%s Note OFF | Pitch %d | Beat %s | Lane %d", Log.ICONS.NOTE_OFF, note.pitch, note.time, lane.lane_num))
...
function Conductor:stop_all_notes(lane)
  ...
  Log.log("CONDUCTOR", "PLAYBACK", string.format("%s Stopping all notes | Lane %d", Log.ICONS.NOTE_OFF, lane.lane_num))
...
function Conductor:schedule_stage(lane, stage)
  ...
  local grid_division, gate_length, denominator
  if is_grid_mode then
    local grid_param = params:lookup_param("lane_" .. lane.lane_num .. "_grid_division")
    denominator = grid_param.options[params:get(grid_param.id)]
    local speed = lane:get_param("grid_speed")
    grid_division = (1/denominator) / speed
    gate_length = lane:get_param("gate_length") * 0.1
  end
  
  -- Calculate total duration for each loop iteration
  local loop_duration
  if is_grid_mode then
    -- Grid mode: Calculate from note count and grid division
    loop_duration = lane.motif.note_count * grid_division
  else
    -- Free mode: Use the total recorded duration from motif
    loop_duration = lane.motif.recorded_duration
  end
  
  local total_loop_duration = loop_duration + loop_rest
  ...
  if is_grid_mode then
    local speed = lane:get_param("grid_speed")
    local effective_division = denominator * speed
    local division_display = denominator < 1 
      and string.format("%.2f/1", denominator)
      or string.format("1/%d", denominator)
    Log.log("CONDUCTOR", "BOUNDARY", string.format("%s [GRID] Stage %d ▸ Division: %s (%.3f beats) | Speed: %.2fx | Gate: %.1f%% | Loops: %d | Rest: %.1f,%.1f | Lane %d", Log.ICONS.STAGE, lane.current_stage, division_display, grid_division, speed, gate_length * 100, stage.num_loops, loop_rest, stage_rest, lane.lane_num))
  else
    Log.log("CONDUCTOR", "BOUNDARY", string.format("%s [FREE] Stage %d ▸ Transform: %s | Loops: %d | Rest: %.1f,%.1f | Pattern: %.3f | Total: %.3f | Lane %d", Log.ICONS.STAGE, lane.current_stage, transform_info, stage.num_loops, loop_rest, stage_rest, loop_duration, total_loop_duration, lane.lane_num))
  end
  
  clock.run(function()
    -- Only sync on first stage of playback
    if not lane.initial_sync_done then
      clock.sync(1)
      lane.initial_sync_done = true
      lane.playback_start_beat = clock.get_beats()
      Log.log("CONDUCTOR", "TIMING", string.format("%s Initial sync at beat %s | Lane %d", Log.ICONS.CLOCK, Log.format.beat(lane.playback_start_beat), lane.lane_num))
    end
    
    -- Pre-calculate event table timing
    local events = {}
    local stage_start = lane.playback_start_beat
    
    for loop = 0, stage.num_loops - 1 do
      -- Calculate loop start based on mode
      local loop_start
      if is_grid_mode then
        -- Grid mode: Each loop starts at fixed intervals
        loop_start = stage_start + (loop * total_loop_duration)
      else
        -- Free mode: Each loop starts after full recorded duration plus rest
        loop_start = stage_start + (loop * total_loop_duration)
      end
      ...
      -- Add loop end event at the full recorded duration
      table.insert(events, {
        type = "loop_end",
        time = loop_start + loop_duration,  -- Use full recorded duration
        loop = loop + 1
      })
    end
    
    -- Add stage end event after last loop
    table.insert(events, {
      type = "stage_end",
      time = stage_start + (stage.num_loops * total_loop_duration) + stage_rest,
    ...
    for _, event in ipairs(events) do
      ...
      if event.time ~= last_sync_time then
        Log.log("CONDUCTOR", "TIMING", string.format("%s Syncing to beat %s | Event: %s", Log.ICONS.CLOCK, Log.format.beat(event.time), event.type))
        clock.sync(event.time)
        last_sync_time = event.time
      end
      
      Log.log("CONDUCTOR", "TIMING", string.format("%s Executing at beat %s | Event: %s",Log.ICONS.CLOCK, Log.format.beat(clock.get_beats()), event.type))
      ...
      elseif event.type == "loop_start" then
        Log.log("CONDUCTOR", "BOUNDARY", string.format("%s Loop %d/%d ▸ Beat %s | Lane %d", Log.ICONS.PLAY, event.loop, stage.num_loops, Log.format.beat(event.time), lane.lane_num))
      elseif event.type == "stage_end" then
        -- Handle stage transition at the exact calculated end time
        if lane.is_playing then
            Log.log("CONDUCTOR", "TIMING", string.format("%s Stage end at beat %s | Lane %d", Log.ICONS.CLOCK, Log.format.beat(event.time), lane.lane_num))
            
            -- Queue up next stage info but don't schedule yet
            local next_stage = lane:advance_stage()
            if next_stage then
                Log.log("CONDUCTOR", "BOUNDARY", string.format("%s Queuing stage %d | Lane %d", Log.ICONS.STAGE, next_stage, lane.lane_num))
                lane.next_stage = {
                    number = next_stage,
                    start_time = event.time,  -- Preserve exact timing
                    config = {
                ...
                Log.log("CONDUCTOR", "BOUNDARY", string.format("%s No active stages - stopping | Lane %d", Log.ICONS.STAGE, lane.lane_num))
    ...
    -- After all events are processed, schedule next stage if queued
    if lane.is_playing and lane.next_stage then
        -- Get current beat for next stage start
        local next_start_time = clock.get_beats()
        
        Log.log("CONDUCTOR", "BOUNDARY", 
            string.format("%s Starting queued stage %d at beat %s | Lane %d", 
                Log.ICONS.STAGE, lane.next_stage.number, 
                Log.format.beat(next_start_time), lane.lane_num))
        
        -- Set start time for next stage
        lane.playback_start_beat = next_start_time
        
        -- Schedule the queued stage
        self:schedule_stage(lane, lane.next_stage.config)
...
function Conductor:play_lane(lane_num)
  ...
  if not lane.motif then 
    Log.log("CONDUCTOR", "STATUS", string.format("%s No motif to play | Lane %d", Log.ICONS.STOP, lane_num))
    return 
  end
  
  Log.log("CONDUCTOR", "STATUS", string.format("%s Starting | Lane %d", Log.ICONS.PLAY, lane_num))
...
function Conductor:stop_lane(lane_num)
  ...
  Log.log("CONDUCTOR", "STATUS", string.format("%s Stopping | Lane %d", Log.ICONS.STOP, lane_num))
  
  self:stop_all_notes(lane)
  lane.is_playing = false
  lane.initial_sync_done = false  -- Reset sync state
  lane.playback_start_beat = nil  -- Reset start beat
end
...
function Conductor:create_motif(lane_num, recorded_data)
  ...
  local motif = Motif.new({
    notes = recorded_data.notes,
    recorded_duration = recorded_data.recorded_duration,
    lane = lane_num
  })
...
function Conductor:add_transform(lane_num, transform_fn, params)
  ...
  Log.log("CONDUCTOR", "TRANSFORM", string.format("%s Transform added: %s | Lane %d", Log.ICONS.TRANSFORM, transform_fn.name or "unnamed", lane_num))
...
function Conductor:sync_transform(lane_nums, transform_fn, params)
  Log.log("CONDUCTOR", "TRANSFORM", string.format("%s Syncing transform %s across lanes [%s]", Log.ICONS.TRANSFORM, transform_fn.name or "unnamed", table.concat(lane_nums, ",")))
